...
- Envoyer les *props* depuis le `RouterLink` d'une recette vers la page dédiée à la recette.

En soit, on pourrait aussi récupérer le paramètre de la route, comme l'identifiant, via le `$route.params.id` dans la page dédiée à la recette, et chercher dans notre fichier `recipes.js` la recette correspondante. Mais on va utiliser les *props* pour faire ça, et joindre directement la recette.
